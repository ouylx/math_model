...
def detect_conflicts(paths):
    ...
    for t in range(max_time):  # 点冲突检测
        ...
        for i, path in enumerate(paths):
            if t <= len(path) - 1:  # 到终点后下一个时间步长机器消失，既不检查点冲突
                pos = path[t]
                if pos in occupied_positions:
                    conflicts.append({'type': 'vertex', 'step': t, 'robots': [i, occupied_positions[pos]]})
                ...
                for j, path_j in enumerate(paths):
                    ...
                    if t < len(path_j) - 1:
                        if path_i[t] == path_j[t + 1] and path_i[t + 1] == path_j[t]:
                            conflicts.append({'type': 'edge', 'step': t + 1, 'robots': [i, j]})
    if conflicts:
