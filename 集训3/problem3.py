    ...
    while open_list:
        ...
        for robot, step in zip(robots, steps):
            ...
            new_paths[robot] = calculate_constraint_path(robot, grid, agent_pos, st, roles,
                                                         new_constraints.get(robot), new_len_list[robot])[0]  # 添加冲突约束重新计算
...
if __name__ == "__main__":
    ...
    if ok:
        time1 = time.time()
        assignment_roles, _ = Greedy3.greedy_main(dist, dist_start)  # 贪心算法求解任务分配
        #_, assignment_roles = DP3_12.tsp_single_robot(dist, dist_start)  # 动态规划求解任务分配
